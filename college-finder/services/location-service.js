/**
 * Location Service
 * Handles zip code lookups and distance calculations
 */

// US Zip code ranges by state (approximate - first 3 digits)
// This allows us to determine state from zip code without an external API
const ZIP_TO_STATE = {
    // Alabama
...
// Approximate lat/long for zip code prefixes (major areas)
const ZIP_COORDINATES = {
    // Major city coordinates by first 3 digits of zip
    ...
    '782': { lat: 29.4241, lon: -98.4936 },  // San Antonio
    '921': { lat: 32.7157, lon: -117.1611 }, // San Diego
    '752': { lat: 32.7767, lon: -96.7970 },  // Dallas
    '941': { lat: 37.7749, lon: -122.4194 }, // San Francisco
    '787': { lat: 30.2672, lon: -97.7431 },  // Austin
    ...
    '024': { lat: 42.3601, lon: -71.0589 },  // Boston area
    '481': { lat: 42.3314, lon: -83.0458 },  // Detroit
    '551': { lat: 44.9778, lon: -93.2650 },  // Minneapolis
};
...
function getCoordinatesFromZip(zipCode) {
    ...
    // Try to find nearest prefix we have
    const state = ZIP_TO_STATE[prefix];
    if (state) {
        // Return approximate state center coordinates
 ...
 */
function getStateCenterCoordinates(state) {
    const stateCenters = {
...
/**
 * Check if user is in-state for a college
 */
function isInState(userZipCode, collegeState) {
    const userState = getStateFromZip(userZipCode);
    return userState === collegeState;
}

/**
 * Get full location info from zip code
 */
